// Iteration #1: Find the maximum
//Implement the function maxOfTwoNumbers that takes two numbers as arguments and returns the bigger number.
//const maxOfTwoNumbers= (num1, num2)=>{}
// function maxOfTwoNumbers(num1, num2) {
//   if(num1>num2){
//     return num1;
//   }
//   else{
//     return num2;
//   }
// };
// console.log(maxOfTwoNumbers(2, 3));


//--------Diego
//new CDeclaration, variable que contien una funcion
//

//--------Terniario como Alvarito


// function maxOfTwoNumbers(a, b) {
//   return a > b ? a : b;
// }
//la comparativa, caso exitoso y caso negativo( lo que vamos a expandir)
// const maxOfTwoNumbersmultiple= (a,b) => {
// return a > b ? a : a === b ? a : b}
//console.log("")

function maxOfTwoNumbers(num1,num2){
  if(num1>num2){
      return num1;
  }
  else{
      return num2;
  }
}

console.log(maxOfTwoNumbers(2,4));
console.log(maxOfTwoNumbers(2,2));
console.log(maxOfTwoNumbers(3,2));



//Iteration #2: Find the longest word

//Implement the function findLongestWord that takes as an argument an array of words and returns the longest one. 
// If there are 2 with the same length, it should return the first occurrence.

//You can use the following array to test your solution:


const words = ['mystery', 'brother', 'aviator', 'crocodile', 'pearl', 'orchard', 'crackpot'];
function findLongestWord(wordsArr){
    let longest = "";
    for(let i=0;i<wordsArr.length;i++){
        let wordLength = wordsArr[i].length;
        if(wordLength>longest.length){
            longest= wordsArr[i];
        }
    }
    return longest;
}


console.log(findLongestWord(words));


//---diego---- 


//Conditional + for y buscar/loopear la longitus de cada string

//const findLongestWord = (wordArray) => {
  //if (wordArray).length===0= return null; si es cero , no me devuelvas nada?? null??
  //let longestWord;
  //for ( let i=0:i< wordArray.length;i++){
    //if(longestWord.length< wordArray[i].length) longestWord=wordArray[i]
  //}
  //return longestWord;
//}


//----otra version---------


// const findLongestWordSecondVariation = (wordArray) => { 
//   let wordLength="";
// // el simbolo ! nos devuelve la viceversa de algo, si no hay longitud !
//   if(!wordArray.length) return null;
//   else if(wordArray.length===1) return wordArray.toString()
//   else {
//     let temporalWord= wordArray.sort((wordOne, wordTwo)==>
//      wordOne.legth> WIRDtWO.LENGTH ? -1 : 1
//      );
//     wordLength=temporalWord[0];
//   }
//   return wordLength;
//};



// Iteration #3.1: Sum numbers
// Calculating a sum can be as simple as iterating over an array and adding each of the elements together.

// Implement the function named sumNumbers that takes an array of numbers as an argument, and returns the sum of all of 
// the numbers in the array. Later in the course, we will learn how to do this by using the reduce array method, 
// which will make your work significantly easier. For now, let's practice the "declarative" way of adding values, using loops.

// You can use the following array to test your solution:

// const numbers = [6, 12, 1, 18, 13, 16, 2, 1, 8, 10];


// function sumNumbers(arrNum) {
//   let sum=0;
//   for(let i=0;i<arrNum.length;i++){
//     sum= sum + arrNum[i];
//   }
//   return sum;
// }

// sumNumbers(numbers);
// console.log("La sumatoria del array: " + sumNumbers(numbers));

const numbers = [6, 12, 1, 18, 13, 16, 2, 1, 8, 10];


function sumNumbers(numbersArr){
    let suma = 0;
    for (let i=0; i<numbersArr.length;i++){
        let num = numbersArr[i];
        suma= suma + num;
        //suma += num;
    }
    return suma;
};




console.log(sumNumbers(numbers));



// Iteration #3.1 Bonus:
// const sum = (mixedArray) => {
//   let totalSumofMixedArrayPositions=0;
//   //bucleeemos
//   for(let i=0;i<mixedArray;i++){
//     //string
//     if(typeof mixedArray[i] ==="string"){
//       let string=mixedArray[i].split("");
//       for(let i=0;i<string.length;i++){
//         totalSumofMixedArrayPositions += string[i].length;
//       }
//     }
//     //booleanos
//     else if(mixedArray[i]=== true) {
//       totalSumofMixedArrayPositions +=1;

//     }
//     //resto aka numbers
//     else{
//       totalSumofMixedArrayPositions += mixedArray[i];
//     }
//   }
//   return totalSumofMixedArrayPositions;
// }
// console.log('the sum of this mixed array is: ${sum(mixedArr)}');

//alvarito : totalSumOfMixedArrayPositions += mixedArray[i].length;


function sum() {}



// Iteration #4: Calculate the average

// Find the sum as we did in the first exercise (or how about reusing the function sumNumbers()?)
// Take that sum and divide it by the number of elements in the array.


let media = sumNumbers(numbers)/numbers.length;

console.log(media);
// Implement the function averageNumbers that expects an array of numbers and returns the average of the numbers.
// You can use the following array to test your solution:


// // Level 1: Array of numbers

const numbers2 = [2, 6, 9, 10, 7, 4, 1, 9];
function averageNumbers(numbersArr){
    let average = sumNumbers(numbersArr)/numbersArr.length;
return average;

}
console.log(averageNumbers(numbers2));

// Level 2: Array of strings

const words42 = ['seat', 'correspond', 'linen', 'motif', 'hole', 'smell', 'smart', 'chaos', 'fuel', 'palace'];

function averageWordLength(wordsArr) {
    const lengthsArr= []; //esta la creamos para crear un array con numeros, que son las longitudes de las palabras
    //conseguir el array de longitudes:
    for(let i = 0; i< wordsArr.length; i++){
        let wordLenght= wordsArr[i].length;
        lengthsArr.push(wordLenght);
       //tenemos un array con las longitudes de esta palabra , al acabar el loop
    }
    return averageNumbers(lengthsArr);
}

console.log("Average word lenght:" + averageWordLength(words42));

// Bonus - Iteration #4.1



//4.2.Bonus

//ternarios------------

// const averageWordLength = (wordsArray) =>{
//   !wordsArray.length ? null : wordsArray.join("").length / wordsArray.length
// }
// console.log('la media de palabras de etse array es:' ${averageWordLength(wordsArr)};


// //--------ternarios alvarito

// const  averageWordLength = (arr) => {
//   return arr.join('').length / arr.length      
//  }
function avg(){} // Para que no haya error

// Iteration #5: Unique arrays
const wordsUnique = [
  'crab',
  'poison',
  'contagious',
  'simple',
  'bring',
  'sharp',
  'playground',
  'poison',
  'communion',
  'simple',
  'bring'
];

function uniquifyArray(wordsArr){
  const result = [];
  for(let i = 0; i<wordsArr.length;i++){
      let word= wordsArr[i];
      let index= wordsArr.indexOf(word);
      if(i === index) {
          result.push(word);
      }
  }
  return result;
}

console.log(uniquifyArray(wordsUnique));




// Iteration #6: Find elements
const words3 = ['machine', 'subset', 'trouble', 'starting', 'matter', 'eating', 'truth', 'disobedience'];

function doesWordExist(wordArray, word) {
  return wordArray.includes(word);
}


console.log(doesWordExist(words3,"pachamama"));
console.log(doesWordExist(words3, "machine"));


// Iteration #7: Count repetition
const words7 = [
  'machine',
  'matter',
  'subset',
  'trouble',
  'starting',
  'matter',
  'eating',
  'matter',
  'truth',
  'disobedience',
  'matter'
];

function howManyTimes(wordsArr,word){
  let count =0;
  for(let i=0;i< wordsArr.length;i++){
      if(wordsArr[i]===word){
          count = count + 1;
      }
  }
  return count;
}

console.log("the word appears : " + howManyTimes(words7, "matter") + " times");



// Iteration #8: Bonus
const matrix = [
  [8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8],
  [49, 49, 99, 40, 17, 81, 18, 57, 60, 87, 17, 40, 98, 43, 69, 48, 4, 56, 62, 0],
  [81, 49, 31, 73, 55, 79, 14, 29, 93, 71, 40, 67, 53, 88, 30, 3, 49, 13, 36, 65],
  [52, 70, 95, 23, 4, 60, 11, 42, 69, 24, 68, 56, 1, 32, 56, 71, 37, 2, 36, 91],
  [22, 31, 16, 71, 51, 67, 63, 89, 41, 92, 36, 54, 22, 40, 40, 28, 66, 33, 13, 80],
  [24, 47, 32, 60, 99, 3, 45, 2, 44, 75, 33, 53, 78, 36, 84, 20, 35, 17, 12, 50],
  [32, 98, 81, 28, 64, 23, 67, 10, 26, 38, 40, 67, 59, 54, 70, 66, 18, 38, 64, 70],
  [67, 26, 20, 68, 2, 62, 12, 20, 95, 63, 94, 39, 63, 8, 40, 91, 66, 49, 94, 21],
  [24, 55, 58, 5, 66, 73, 99, 26, 97, 17, 78, 78, 96, 83, 14, 88, 34, 89, 63, 72],
  [21, 36, 23, 9, 75, 0, 76, 44, 20, 45, 35, 14, 0, 61, 33, 97, 34, 31, 33, 95],
  [78, 17, 53, 28, 22, 75, 31, 67, 15, 94, 3, 80, 4, 62, 16, 14, 9, 53, 56, 92],
  [16, 39, 5, 42, 96, 35, 31, 47, 55, 58, 88, 24, 0, 17, 54, 24, 36, 29, 85, 57],
  [86, 56, 0, 48, 35, 71, 89, 7, 5, 44, 44, 37, 44, 60, 21, 58, 51, 54, 17, 58],
  [19, 80, 81, 68, 5, 94, 47, 69, 28, 73, 92, 13, 86, 52, 17, 77, 4, 89, 55, 40],
  [4, 52, 8, 83, 97, 35, 99, 16, 7, 97, 57, 32, 16, 26, 26, 79, 33, 27, 98, 66],
  [88, 36, 68, 87, 57, 62, 20, 72, 3, 46, 33, 67, 46, 55, 12, 32, 63, 93, 53, 69],
  [4, 42, 16, 73, 38, 25, 39, 11, 24, 94, 72, 18, 8, 46, 29, 32, 40, 62, 76, 36],
  [20, 69, 36, 41, 72, 30, 23, 88, 34, 62, 99, 69, 82, 67, 59, 85, 74, 4, 36, 16],
  [20, 73, 35, 29, 78, 31, 90, 1, 74, 31, 49, 71, 48, 86, 81, 16, 23, 57, 5, 54],
  [1, 70, 54, 71, 83, 51, 54, 69, 16, 92, 33, 48, 61, 43, 52, 1, 89, 19, 67, 48]
];

function greatestProduct() {}




// The following is required to make unit tests work.
/* Environment setup. Do not modify the below code. */
if (typeof module !== 'undefined') {
  module.exports = {
    maxOfTwoNumbers,
    findLongestWord,
    sumNumbers,
    sum,
    averageNumbers,
    averageWordLength,
    avg,
    uniquifyArray,
    doesWordExist,
    howManyTimes,
    greatestProduct
  };
}
